...
const [err, setErr] = React.useState(undefined);
  const handleSubmit = (event) => {
   ...
   // rediracted to dashbord 

   if(error.errors.msg){
     setErr(error.errors.msg)
   }
   else{setErr(error.errors[0].msg)}
  }
        ...
        <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square>
          ...
          >
            ...
            <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 1 }}>
              ...
              {err=== undefined? null:<Alert variant="filled" severity="error" onClick={() => {handleClose()}}>{err}</Alert>
}
              <Copyright sx={{ mt: 5 }} />
            </Box>
          </Box>
